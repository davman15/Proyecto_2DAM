                    ...
                    querySnapshot.forEach((doc) => {
                        ...
                        var contenido = '<div class="col-12 text-center" style="margin-left:0px; height:90%;"> ' +
                            ...
                            '<div class="col-11 centrarBoton fondo row " style="height:80%;" >' +
                                '<div  class="scrollDiv col-12 bg-danger h-100"  id="divIzquerda-'+doc.data().usuarioId+'">' +
                                '</div>' +
                            '</div>' +
    ...
    if (!(inputChat.value.length == 0)) {
        ...
        firebase.auth().onAuthStateChanged(function (user) {
            if (user) {
                ...
                var usuarioRef = db.collection('Usuarios').doc(user.displayName).collection('chats').doc(idChat).set({
                    fechaChat: fecha,
                    id: idChat,
                    ...
                    .then(() => {
                        ...
                        var query2=chatRef.orderBy("fechaChat","desc")
                            .get()
                            .then((querySnapshot)=>{
                                querySnapshot.forEach((doc)=>{
                                    var idschats=doc.id+"";
